...
impl SSHClientKey {
    ...
    pub fn make_ssh_handshake(&mut self, rs: String, un: String, pw: String, pubk: String, privk: String) -> Result<i8, HandshakeErrorCode> {
        ...
        if self.public_key == "" {
        //Attempt to authenticate a login with a key 
            match session_attempt.userauth_pubkey_file(&self.username, None, Path::new(&self.private_key), Some(&self.password)) {
                Ok(()) => {
            ...
            match session_attempt.userauth_pubkey_file(&self.username, Some(Path::new(&self.public_key)), Path::new(&self.private_key), Some(&self.password)) {
                Ok(()) => {
                    if !session_attempt.authenticated() {
                        self.session_still_valid = false;
                        return Err(HandshakeErrorCode::SessionAuthFail);
